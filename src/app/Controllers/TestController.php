...
class TestController extends Controller {
	...
	public function getGeneralTest() {
        if(\App::environment('local')){
            ...
            $items = \Solunes\Reservation\App\FieldTranslation::where('label', 'like', '%fields.%')->groupBy('label')->orderBy('label')->get();
            if(count($items)>0){
            ...
            $items = \Solunes\Reservation\App\FieldOptionTranslation::where('label', 'like', '%admin.%')->groupBy('label')->orderBy('label')->get();
            if(count($items)>0){
            ...
            $items = \Solunes\Reservation\App\MenuTranslation::where('name', 'like', '%admin.%')->groupBy('name')->orderBy('name')->get();
            if(count($items)>0){
            ...
            $items = \Solunes\Reservation\App\Field::whereIn('parent_id', $nodes_array)->where('type', 'text')->get();
            if(count($items)>0){
